...
router.get("/", requireAuth, requireRole(["admin", "super_admin"]), async (_req: Request, res: Response) => {
...
router.post("/", requireAuth, requireRole(["admin", "super_admin"]), async (req: Request, res: Response) => {
  ...
  await createAuditLog({ action: "user_created", actorUserId: req.user!.userId, targetType: "user", targetId: id, details: `Created user ${email}` });
...
router.put("/:id", requireAuth, requireRole(["admin", "super_admin"]), async (req: Request, res: Response) => {
  ...
  await createAuditLog({ action: "user_updated", actorUserId: req.user!.userId, targetType: "user", targetId: req.params.id, details: "User updated" });
  res.json({ message: "Updated" });
});

router.delete("/:id", requireAuth, requireRole(["admin", "super_admin"]), async (req: Request, res: Response) => {
  if (req.params.id === req.user!.userId) { res.status(400).json({ message: "Cannot delete yourself" }); return; }
  await pool.query("DELETE FROM users WHERE id = $1", [req.params.id]);
  await createAuditLog({ action: "user_deleted", actorUserId: req.user!.userId, targetType: "user", targetId: req.params.id, details: "User deleted" });
