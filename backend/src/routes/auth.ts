...
router.post("/mfa/setup", requireAuth, async (req: Request, res: Response) => {
  const result = await pool.query("SELECT email, mfa_enabled FROM users WHERE id = $1", [req.user!.userId]);
  ...
  await pool.query("UPDATE users SET mfa_secret = $1 WHERE id = $2", [secret, req.user!.userId]);
...
router.post("/mfa/verify", requireAuth, async (req: Request, res: Response) => {
  ...
  const result = await pool.query("SELECT mfa_secret FROM users WHERE id = $1", [req.user!.userId]);
  ...
  await pool.query("UPDATE users SET mfa_enabled = TRUE WHERE id = $1", [req.user!.userId]);
  await createAuditLog({ action: "mfa_enabled", actorUserId: req.user!.userId, targetType: "user", targetId: req.user!.userId, details: "MFA enabled" });

  res.json({ message: "MFA enabled successfully" });
});

router.post("/mfa/disable", requireAuth, async (req: Request, res: Response) => {
  await pool.query("UPDATE users SET mfa_enabled = FALSE, mfa_secret = NULL WHERE id = $1", [req.user!.userId]);
  await createAuditLog({ action: "mfa_disabled", actorUserId: req.user!.userId, targetType: "user", targetId: req.user!.userId, details: "MFA disabled" });
...
router.post("/change-password", requireAuth, async (req: Request, res: Response) => {
  ...
  const result = await pool.query("SELECT password_hash, auth_provider FROM users WHERE id = $1", [req.user!.userId]);
  const user = result.rows[0];
  ...
  await createAuditLog({ action: "password_changed", actorUserId: req.user!.userId, targetType: "user", targetId: req.user!.userId, details: "Password changed" });
